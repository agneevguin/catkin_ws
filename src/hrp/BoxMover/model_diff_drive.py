...
C4=2.5 #controlled zones acquire
D4=1.7 #dead zones acquire
...
class LawnMower:
	...
	def search(self):
		#print "box (%f,%f)" %(self.box_pos[0],self.box_pos[1])
		#box_location = BoxLocation(x=self.box_pos[0],y=self.box_pos[1])
		#goal=Point(x=self.goal[0],y=self.goal[1])
		#point = Motion.get_point_behind_box(box_location, 0.8, goal)
		#return np.array([point.x, point.y,self.box_pos[2] ])
		return self.box_pos
